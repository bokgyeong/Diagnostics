...
# niters = 200000 * thin + burn
niters = 400000 * thin + burn
# niters = 500000 * thin + burn
# niters = 700000 * thin + burn
updateCOV = TRUE
adaptInterval = 200
...
Trunc = c(); rtime = 0; preniters = 0; Accprob = 0

# load(paste0('ACDAIKS/Trunc_knu/bids2Trunc', summax, '.RData'))
# preniters = nrow(Trunc)

sourceCpp('RcppFtns.cpp')

ptm = proc.time()[3]
dummy = appxCOMPreg_knu(outer = niters - preniters, y = y, X = X, beta = beta_initial, nu = nu,
                        block = block, sigma2 = sigma2, COV = COV, updateCOV = updateCOV, 
                        adaptInterval = adaptInterval, adaptFactorExponent = adaptFactorExponent,
                        adapIter = adapIter, summax = summax, thin = thin, num = ncores)
rtime = rtime + proc.time()[3] - ptm

Trunc = rbind(Trunc, dummy$Sample)
...
save(Trunc, Accprob, rtime, thin, block, ncores, burn,
     sigma2, adapIter, COV, beta_initial, 
     updateCOV, adaptInterval, adaptFactorExponent, nu,
     file = paste0('ACDAIKS/Trunc_knu/bids2Trunc', summax, '.RData'))
     # file = paste0('ACDAIKS/Trunc_knu/bids2Trunc', summax, 'v2.RData'))
