...
# for ACD and AIKS
# ==============================================================================
# unbiased and consistent approximations
# ------------------------------------------------------------------------------
...
#   Sybar = colMeans(Sy)
#   res =  -mean(Sy^2) + ( Sybar[1]*Sybar[2] + Sybar[1]*Sybar[3] + Sybar[2]*Sybar[3] ) / 3
#   return(res)
# }
# 
# 
...
#   Sybar = colMeans(Sy)
#   res = Sx*Sx - 2*Sx*mean(Sy) + ( Sybar[1]*Sybar[2] + Sybar[1]*Sybar[3] + Sybar[2]*Sybar[3] ) / 3
#   return(res)
# }
# 
# ### approximate derivative of r_1(\theta)
# f_Ahat = function(Sx, Sy){
#   Sybar = colMeans(Sy)
#   Sy2bar = colMeans(Sy * Sy)
#   Sy3bar = colMeans(Sy * Sy * Sy)
#   res = -2 * Sx * ( mean(Sy2bar) - ( Sybar[1]*Sybar[2] + Sybar[1]*Sybar[3] + Sybar[2]*Sybar[3] ) / 3 ) + 
#     5 * ( Sybar[1]*Sy2bar[2] + Sybar[1]*Sy2bar[3] + Sybar[2]*Sy2bar[1] + Sybar[2]*Sy2bar[3] + Sybar[3]*Sy2bar[1] + Sybar[3]*Sy2bar[2] ) / 6 -
#     4 * prod(Sybar) - mean(Sy3bar)
#   return(res)
# }
# 
# 
...
#   Jbar = colMeans(J)
#   
#   ABH = rbind(Abar, Bbar, Hbar)
#   dummy = permutations(5, 3)
#   V1 = mean(sapply(1:nrow(dummy), function(i) Abar[dummy[i,1]] * Bbar[dummy[i,2]] / Hbar[dummy[i,3]] ))
#   dummy2 = permutations(5, 5)
#   V2 = mean(sapply(1:nrow(dummy2), function(i) Abar[dummy2[i,1]] / Hbar[dummy2[i,2]] * Jbar[dummy2[i,3]] / Hbar[dummy2[i,4]] * Abar[dummy2[i,5]] ))
#   
#   V = mean(r * r) - 2*V1 + V2
#   return(V)
# }
# 
# 
# ### ACD using the unbiased estimate of V
# ACD = function(U, H, J, A){
#   n = length(as.vector(U))
#   rbar = mean(J + H)
#   
#   res = n * (rbar^2) / f_Vhat(U, H, J, A)
#   return(res)
# }
...
### approximate H matrix
f_Hhat_b = function(Sx, Sy){
  Sybar = mean(Sy)
  return(-mean(Sy^2) + Sybar * Sybar)
}


### approximate derivative of r_1(\theta)
f_Ahat_b = function(Sx, Sy){
  Sybar = mean(Sy)
  Sy2bar = mean(Sy * Sy)
  Sy3bar = mean(Sy * Sy * Sy)
  return( -2 * Sx * ( Sy2bar - Sybar^2 ) + 5 * Sybar * Sy2bar - 4 * Sybar^3  - Sy3bar )
}


### biased but consistent estimate of V
f_Vhat_b = function(U, H, A){
  n = length(U)
  J = U * U
  d = J + H
  B = d * d
  ...
  return( Bbar - 2 * Abar / Hbar * Cbar + Abar / Hbar * Jbar / Hbar *Abar )
}


### ACD using the biased but consistent estimate of V
ACD_b = function(U, H, A){
  n = length(U)
  dbar = mean(U * U + H)
  
  res = n * (dbar^2) / f_Vhat_b(U, H, A)
  return(res)
}
